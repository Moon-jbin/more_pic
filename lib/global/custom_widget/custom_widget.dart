...
import 'package:more_pic/provider/global_provider.dart';
import 'package:more_pic/provider/search_provider.dart';
import 'package:more_pic/provider/product_db_provider.dart'; // 진짜 가방 주입
import 'package:more_pic/utils/delegate/sliverHeaderDelegate.dart';
import 'package:more_pic/utils/routing/navigation_service.dart';
import 'package:more_pic/utils/routing/router_name.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';
...
class CustomScaffold extends HookConsumerWidget {
  final Widget Function(BuildContext context, ScrollController scrollController)
      bodyBuilder;
  final String category;
  final bool showSearchIcon; // 💡 검색 아이콘 노출 여부 플래그
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // 💡 [완치 수식]:useScrollController를 독립적으로 선언하여 공용 컨트롤러 충돌 튕김 버그를 완벽하게 박멸합니다.
    final scrollController = useScrollController();
    final showButton = useState(false);
    final isScrolled = useState(false);

    // 🌟 [완치 포인트 1]: 에러의 원흉이던 productManagementProvider 추적 코드를 완전히 치우고,
    // 정석대로 현재 화면 방 주소(category)의 진짜 데이터 가방을 관찰(watch)합니다. String 타입 연동 성공!
    final paginatedState = ref.watch(paginatedProductProvider(category));

    // 🌟 [완치 포인트 2]: List<dynamic> 캐스팅 경고등을 파괴하기 위해 .cast<ProductModel>() 수식을 얹고
    // 가방이 로딩 중이거나 빌 때 터지지 않도록 빈 배열([]) 가드를 단단히 쳤습니다.
          ...
          children: [
            CustomScrollView(
              ...
              slivers: [
                ...
                // 📌 섹션 C: 본문 알맹이 (ProductListPage 그리드가 복사될 캔버스)
                SliverToBoxAdapter(
                  child: bodyBuilder(context, scrollController),
                ),
              ],
            ),
            // 🌟 청정 정제 완료된 진짜 상품 데이터를 슬라이딩 서치바로 안전 배달 도킹! 에러 완전 멸종!
            SlidingSearchBar(currentScreenItems: filterProducts)
          ],
        ),
...
class CustomWidget {
  ...
  static Widget buildInfoWrap() {
    return Wrap(
      ...
      runSpacing: 8,
      children: [
        _buildInfoText('상호명', '몬엔그레이스'),
        _buildInfoText('대표자명', '문지민'),
        _buildInfoText('사업자 주소', '경기도 남양주시 진접읍 해밀예당로 130'),
        _buildInfoText('대표 전화', '01054907886'),
        _buildInfoText('사업자 등록번호', '5430204088'),
        _buildInfoText('통신판매업 신고번호', '미공개'),
        _buildInfoText('개인정보보호책임자', '몬엔그레이스'),
      ],
    );
  }
      ...
      children: [
        ...
        Row(
          children: [
            Text('대표 ',
                style: TextStyle(color: Colors.grey.shade700, fontSize: 12)),
            const Text('카카오뱅크 3333-37-7919709 문지민 몬엔그레이스',
                style: TextStyle(fontSize: 12)),
          ],
        ),
      ],
